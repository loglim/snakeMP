...
public class LobbyController {
    ...
    // Constants
    private static final int NICKNAME_LENGTH_MIN = 3;
    ...
    private void checkGameStart() {
        int length = nicknameTextField.getText().length();
        boolean disable = false;

        if (length < NICKNAME_LENGTH_MIN || length > NICKNAME_LENGTH_MAX) {
            disable = true;
            hintLabel.setTextFill(Color.ORANGERED);
            if (length < NICKNAME_LENGTH_MIN) {
                hintLabel.setText(String.format("Your nickname must be at least %d characters!", NICKNAME_LENGTH_MIN));
            } else {
                hintLabel.setText(String.format("Your nickname can't be more than %d characters!", NICKNAME_LENGTH_MAX));
            }
        }
        else {
            hintLabel.setText("");
